## 开发路线
优先支持 typeScript + less + css module 开发模式中的 jsx 和 css，生成的页面代码是index.tsx和style.less。

其他支持（按优先级排序，从上到下）：

typeScript + less 开发模式中的 jsx 和 css，生成的页面代码是index2.tsx和style.scss。


```
本DSL仅生成简单的组件，不处理相关数据绑定。用于加速页面UI开发
